...
// 3. Reply Number Listener (භාවිතා කරන්නා අංකයක් රිප්ලයි කළ විට ක්‍රියාත්මක වීමට)
if (typeof conn.ev?.on === 'function') {
    conn.ev.on('messages.upsert', async (chatUpdate) => {
        try {
            ...
            const quotedMsgId = quoted.stanzaId;
            const from = mek.key.remoteJid;

            // Check if the replied message has stored wallpaper choices
            if (global.numberStore && global.numberStore[quotedMsgId]) {
                const choice = body.trim();
                const imageUrl = global.numberStore[quotedMsgId][choice];
                ...
                if (imageUrl) {
                    ...
                    await conn.sendMessage(from, { 
                        image: { url: imageUrl }, 
                        caption: `✅ *𝑯𝑫 𝑾𝒂𝒍𝒍𝒑𝒂𝒑𝒆𝒓 𝑫𝒆𝒍𝒊𝘃𝒆𝒓𝒆𝒅!* \n\n> © 𝑸𝒖𝒆𝒆𝒏 𝑴𝒆𝒏𝒂 𝑴𝒅` 
                    }, { quoted: mek });
            ...
            console.error("NUMBER_STORE_ERROR:", err);
        }
    });
}
